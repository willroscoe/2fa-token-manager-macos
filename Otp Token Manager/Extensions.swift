...
//  Totp Token Manager
//
//  Created by William Roscoe on 2017/05/10.
...
extension Data {
    ...
    func hmac(algorithm: CryptoAlgorithm, keyData: Data) -> Data {
        
        let keyLen:Int = Int(keyData.count)
        
        let keyDataUnsafe = keyData.withUnsafeBytes { UnsafePointer<UInt8>($0) }
        
        var result = Data(count: Int(algorithm.digestLength))
        _ = result.withUnsafeMutableBytes {resultPtr in
            self.withUnsafeBytes {(bytes: UnsafePointer<UInt8>) in
                CCHmac(algorithm.HMACAlgorithm, keyDataUnsafe, keyLen, bytes, count, resultPtr)
            }
        }
